...
use crate::mem::addr::ppn;
/// Sv39 页表项 (PTE)
#[derive(Clone, Copy)]
pub struct Pte(pub usize); // 64 位
...
impl Pte {
    pub fn new(pa: usize, flags: usize) -> Self {
        Pte((ppn(pa) << 10) | (flags & 0x3FF)) // 44 位物理页号 + 10 位标志
    }
...
impl PageTable {
    ...
    pub fn map(&mut self, vpn: usize, pa: usize, flags: usize) {
